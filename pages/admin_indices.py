...
def show_admin_indices():
    ...
    with st.expander("➕ Adicionar Novo Índice", expanded=False):
        ...
        with c_new1:
            new_idx_name = st.text_input("Nome", placeholder="Ex: IGPM", label_visibility="collapsed", key="new_name_input")
        with c_new2:
            new_idx_val = st.number_input("Valor", min_value=0.0, step=0.01, format="%.2f", label_visibility="collapsed", key="new_val_input")
        with c_new3:
    ...
    for idx in indices:
        ...
        with c1:
            edited_name = st.text_input("Nome", value=idx['name'], label_visibility="collapsed", key=f"name_{idx['id']}")
        with c2:
            edited_val = st.number_input("Valor", value=float(idx['value']), step=0.01, format="%.2f", label_visibility="collapsed", key=f"val_{idx['id']}")
        with c3:
